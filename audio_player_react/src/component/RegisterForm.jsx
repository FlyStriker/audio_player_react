        ...
        <div className="register_convas">
            <div className="register_form">
                ...
                <div className="register_form_button">
                    <button onClick={() => {
                        callRegister({variables: {login, password}}).then(response => {
                            ...
                            if(result){
                                callLogin({variables: {login, password}}).then(response => {
                                    if(!response.data.login){
                                        alert("Пользователя не существует");
                                        return;
                                    }
                                    SessionStorage.createSession(login, response.data.login);
                                    navigator("/");
                                });
                            }
                        })
                    ...
                    }} className="btn_register">Register</button>
                    <Link to="/login" className="link_to">Log In</Link>
                </div>
            </div>
        </div>
    );
